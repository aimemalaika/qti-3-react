...
export const Player = ({
  ...
  items: { href: string; identifier: string }[], assets: string
}) => {
  const server = `${assets}`;
  ...
  useEffect(() => {
    ...
    const fetchItem = async () => {
      const xmlFetch = await fetch(
        `${server}/${items[itemIndex]?.href}`
      );
      const xml = await xmlFetch.text();

      console.log(`Fetching item ${itemIndex}: ${items[itemIndex]?.href}`);
  ...
  console.log(`Item ${itemIndex}: ${items[itemIndex]?.href}`);
  ...
  return (
    <div className="w-screen h-screen px-8 bg-gray-100 rounded-2xl flex flex-col items-center justify-center">
      <h1 className="text-4xl text-center font-bold">
        {items[itemIndex]?.identifier.replace('_', ' ').toLocaleUpperCase()}
      </h1>
      <QtiItem
